...
public class Minimax {
    ...
    private static int minimax(Board board, TicTacToeScorer scorer, Player player, int depth)
    {
        ...
        if(scorer.isGameOver() || depth <= 0) {
			...
			for(int move : potentialMoves) {				
				...
				bestOpponentScore = Math.max(minimax(board, scorer, opposingPlayer, depth - 1), bestOpponentScore);
				board.markPosition('\0', move);
